...
A project often depends on third-party libraries. For example, 2Do depends on the http://wiki.fasterxml.com/JacksonHome[Jackson library] for XML parsing and the http://www.ocpsoft.org/prettytime[PrettyTime library] for natural language handling. Managing these _dependencies_ can be automated using Gradle. For example, Gradle can download the dependencies automatically, which is better than these alternatives. +
