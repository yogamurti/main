...
* Depending on your connection speed and server load, it can even take up to 30 minutes for the set up to finish (This is because _Gradle_ downloads library files from servers during the project set up process).
...
.  Enter an arbitrary configuration name e.g. _2Do_
...
The *_Architecture Diagram_* given above explains the high-level design of _2Do_. Given below is a quick overview of each component.
...
* `LogsCenter` : Used by many classes to write log messages to _2Do_'s log file.

The rest of the App consists of four components.

* link:#ui-component[*`UI`*] : The user interface of _2Do_.
* link:#logic-component[*`Logic`*] : The command executor.
* link:#model-component[*`Model`*] : Holds the data of _2Do_ in-memory.
...
_Figure 2.1.2 : Simplified class Diagram of the Logic Component_
...
The `UI` component uses the _JavaFx UI_ framework. The layout of these `UI` parts are defined in matching `.fxml` files that are in the `src/main/resources/view` folder. For example, the layout of the link:{repoURL}/blob/master/src/main/java/teamthree/twodo/ui/MainWindow.java[`MainWindow`] is specified in link:{repoURL}/blob/master/src/main/resources/view/MainWindow.fxml[`MainWindow.fxml`]

*Model-View-Controller Pattern* +
_2Do_ *decouples* its data, presentation, and control logic by separating them into three different components: *Model*, *View* and *Controller*.

* Model: The `UI` component's `ResultDisplay`, `CategoryListPanel`, `TaskListPanel`, `StatusBarFooter` and `HelpWindow` binds themselves to some data in the link:#model[`Model`] component, which stores and maintains data in _2Do_.
* View: The `UI` component is responsible for displaying data, interacting with the user through the link:#logic[`Logic`] component, and getting automatic data updates from the link:#model[`Model`] component.
* Controller: The `UI` component's `CommandBox` is responsible for detecting user command inputs and forwarding them to the link:#logic[`Logic`] component, which executes the commands.

*Observer Pattern* +
_2Do_ utilizes the _observer pattern_ to *avoid direct coupling* between the `UI` component and other components.

The `UI` component's constituents that depends on data from the link:#model[`Model`] component, such as its `TaskListPanel` constituent, requires notification whenever the link:#model[`Model`] component updates.

This is done by the _TaskListChangedEvent_.

Whenever the data in the link:#model[`Model`] component updates, _2Do_ notifies its relevant `UI` constituents by calling a _indicateTaskListChanged()_ operation, which raises a new _TaskListChangedEvent_. This event will call the relevant `UI` constituents to update their data from the link:#model[`Model`] component and display the updated data.
...
.  The command parsing of each specific command is encapsulated as a `CommandParser` object for the their respective parser objects (e.g. DeleteCommandParser).
.  This results in a `Command` object which is executed by the `LogicManager`.
<<<<<<< HEAD
<<<<<<< HEAD
.  The command execution can affect the `Model` (e.g. deleting a task) and/or raise events.
.  The result of the command execution is encapsulated as a `CommandResult` object which is passed back to the `Ui`.
=======
.  The command execution can affect the link:#model[`Model`] (e.g. adding a task) and/or raise events.
.  The result of the command execution is encapsulated as a `CommandResult` object which is passed back to the link:#ui[`UI`].
>>>>>>> e2316be8bb13260459565087a7d96bb8606b2dc9
=======
.  The command execution can affect the link:#model[`Model`] (e.g. deleting a task) and/or raise events.
.  The result of the command execution is encapsulated as a `CommandResult` object which is passed back to the link:#ui[`UI`].
>>>>>>> 3c0b763810c18937d0cf92df331c1ae1e2ed24bd
...
_2Do_ allows users to save their data at a new filepath(File Directory must be available). The filepath can be specified via the Save command and will contain a new _2Do.xml document_. StorageMananger will set the new filepath and save it into Config. _2Do_ will load from and save to the new filepath for current and future sessions until a new filepath is specified. _2Do_ also allows users to load a TaskList file from previous sessions vias the Load command.
...
* See <<UsingGradle#, UsingGradle.adoc>> for how to run tests using _Gradle_.
...
See <<UsingGradle#, UsingGradle.adoc>> to learn how to use _Gradle_ for build automation.
...
A project often depends on third-party libraries. For example, _2Do_ depends on the http://wiki.fasterxml.com/JacksonHome[Jackson library] for XML parsing and the http://www.ocpsoft.org/prettytime[PrettyTime library] for natural language handling. Managing these dependencies can be automated using _Gradle_. _Gradle_ can download the dependencies automatically, which is better than these alternatives: +
...
|`* * *` |new user |see usage instructions |refer to instructions when I forget how to use _2Do_
...
|`* * *` |user |save my to-do list as a readable file |have a back-up copy of my to-do list and be able to view it across multiple devices
...
1. User requests for the help guide
2. System displays the help guide
...
1. User requests to add a floating task with the name field filled and the end field empty
2. System adds the floating task and displays a success message
...
2. System adds the task, with a deadline, and displays a success message
...
2. System adds the event and displays a success message
...
2. System adds the tag and displays a success message
...
3. User requests to delete the task by using its link:#index[index]
4. System deletes the task and displays a success message
...
2. System deletes the tag and displays a success message
...
2. System undoes the last action and displays a success message
...
2. System redoes the last undo command and displays a success message
...
4. System updates the task and displays a success message
...
1. User searches for a task using *list* or *find*
2. System displays all relevant tasks
3. User requests to mark the task as complete by link:#index[index]
4. System updates the task and displays a success message
...
2. System displays all relevant tasks
3. User requests to mark the task as incomplete by link:#index[index]
4. System updates the task and displays a success message
...
3. User requests to change the general options for the session
4. System updates its general options for the session and displays a success message
...
2. System saves the current to-do list, exports it as an _xml_ file in the directory specified and displays a success message
...
1. User requests to load a to-do list xml file into the system
2. System loads the to-do list and displays the loaded to-do list
...
2. System clears the to-do list
