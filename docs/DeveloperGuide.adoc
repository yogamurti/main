...
A project often depends on third-party libraries. For example, _2Do_ depends on the http://wiki.fasterxml.com/JacksonHome[Jackson library] for XML parsing and the http://www.ocpsoft.org/prettytime[PrettyTime library] for natural language handling. Managing these dependencies can be automated using _Gradle_. Gradle can download the dependencies automatically, which is better than these alternatives: +
