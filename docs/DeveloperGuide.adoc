...
The `UI` component's constituents that depends on data from the link:#model[`Model`] component, such as its `TaskListPanel` constituent, requires notification whenever the link:#model[`Model`] component updates. This is done by the _TaskListChangedEvent_ object. Hence, whenever the data in the link:#model[`Model`] component updates, _2Do_ notifies its relevant `UI` constituents by calling a _indicateTaskListChanged()_ operation, which raises a new _TaskListChangedEvent_. This event will call the `UI` constituents to update their data from the link:#model[`Model`] component and display the updated data.
