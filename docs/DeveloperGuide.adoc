...
* User friendly commands, through simply utilizing its short and simple forms to handle tasks
...
* Adding a task is not intuitive as its respective form is cluttered with additional features, which the user can add, that makes it difficult to know which fields are mandatory
* Setting up the calendar is dependent on another calendar application (e.g. _Google Calendar_) as it enables calendar import by other applications but lacks its own native calendar
