...
The UI component's constituents which depends on data from the link:#model[`Model`] component, such as its `TaskListPanel` constituent, requires notification when the link:#model[`Model`] component updates. This is done by the _TaskListChangedEvent_ object. Hence, whenever the data in the link:#model[`Model`] component updates, _2Do_ notifies its relevant `UI` constituents by calling a _indicateTaskListChanged()_ operation, which raises a new _TaskListChangedEvent_. This event will call the `UI` constituents to update its data from the link:#model[`Model`] component and display the updated data.
