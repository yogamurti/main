...
Whenever the data in the link:#model[`Model`] component updates, _2Do_ notifies its relevant `UI` constituents by calling a _indicateTaskListChanged()_ operation, which raises a new _TaskListChangedEvent_ object. This event will call the relevant `UI` constituents to update their data from the link:#model[`Model`] component and display the updated data.
