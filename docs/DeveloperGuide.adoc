...
*Observer Pattern* +
_2Do_ utilizes the _observer pattern_ to *avoid a direct coupling* between the `UI` component and other components.
...
This is done by the _TaskListChangedEvent_ object.

Whenever the data in the link:#model[`Model`] component updates, _2Do_ notifies its relevant `UI` constituents by calling a _indicateTaskListChanged()_ operation, which raises a new _TaskListChangedEvent_ object. This event will call the relevant `UI` constituents to update their data from the link:#model[`Model`] component and therefore, display the updated data.
...
4. System updates its general options for the session
